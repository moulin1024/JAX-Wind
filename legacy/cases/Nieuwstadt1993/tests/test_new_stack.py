...
def test_new_benchmark_exposes_lasd_path() -> None:
    driver = (BENCHMARK / "run_new.py").read_text()
...
def test_lasd_benchmark_uses_lasd_diagnostics() -> None:
    driver = (BENCHMARK / "run_new.py").read_text()
    assert "lasd_diagnostic_fields(" in driver
    assert "LasdAcceptedStepEvent(" in driver
