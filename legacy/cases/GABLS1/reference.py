"""Readers and ensemble interpolation for the official GABLS1 LES data."""
...
def read_reference_set(path: Path) -> ReferenceSet:
    """Read one original Fortran-record formatted A--E data file."""
    ...
    return ReferenceSet(
        ...
        values=dict(zip(columns, arrays, strict=True)),
    )
...
def load_period_sets(
    ...
    """Load every participant available for a profile set and averaging hour."""
...
def interpolate_values(
    ...
    """Interpolate one participant without extrapolating beyond its data."""
...
def ensemble_statistics(stack: np.ndarray) -> dict[str, np.ndarray]:
    """Return pointwise statistics for a participant-by-height array."""
...
def ensemble_on_grid(
    ...
    if not datasets:
        return {}
    variables = tuple(name for name in datasets[0].values if name != coordinate)
    result = {}
    for variable in variables:
        stack = np.stack(
            [interpolate_values(data, coordinate, variable, target) for data in datasets]
        )
