    ...
    def fused_boussinesq_tendency(
        ...
        *,
        ...
        if not common or not (lasd or static):
            raise TypeError(
                "Boussinesq solve requires a fused dealiased SGS model with "
        ...
        rotational_advection = isinstance(
            momentum_model.advection,
            RotationalAdvection,
        )
        ...
        if momentum_context is None:
            fused_kernel = (
                self.flow.fused_rotational_boussinesq
                if rotational_advection
                else self.flow.fused_boussinesq
            )
            common_arguments = (
                velocity.x.payload,
                velocity.y.payload,
            ...
            fused_kernel = (
                self.flow.fused_rotational_boussinesq_from_context
                if rotational_advection
                else self.flow.fused_boussinesq_from_context
            )
