...
def test_gabls1_rejects_removed_sgs_models(tmp_path: Path) -> None:
    invalid = tmp_path / "invalid.toml"
    invalid.write_text(CONFIG.read_text().replace('model = "lasd"', 'model = "amd"'))
    with pytest.raises(ConfigError, match="must be 'lasd'"):
        load_case(invalid)


def test_gabls1_case_selects_dynamic_closure() -> None:
