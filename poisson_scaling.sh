...
MP5_STRENGTH="${MP5_STRENGTH:-1}"
KO6_STRENGTH="${KO6_STRENGTH:-1}"
MOMENTUM_ADVECTION="${MOMENTUM_ADVECTION:-kep4}"
MOMENTUM_REGULARIZATION="${MOMENTUM_REGULARIZATION:-ko6}"
SCALAR_ADVECTION="${SCALAR_ADVECTION:-mp5}"
PRESSURE_DISCRETIZATION="${PRESSURE_DISCRETIZATION:-kep4}"
FPJ2_TIMESTEP_RATIO_LIMIT="${FPJ2_TIMESTEP_RATIO_LIMIT:-2}"
...
run_args=(
    ...
    --mp5-strength "$MP5_STRENGTH"
    --ko6-strength "$KO6_STRENGTH"
    --momentum-advection "$MOMENTUM_ADVECTION"
    --momentum-regularization "$MOMENTUM_REGULARIZATION"
    --scalar-advection "$SCALAR_ADVECTION"
    --pressure-discretization "$PRESSURE_DISCRETIZATION"
    --projection-method fpj2
    --fpj2-timestep-ratio-limit "$FPJ2_TIMESTEP_RATIO_LIMIT"
...
echo "Transport      : momentum=$MOMENTUM_ADVECTION+$MOMENTUM_REGULARIZATION scalar=$SCALAR_ADVECTION"
