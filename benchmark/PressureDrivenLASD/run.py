#!/usr/bin/env python3
"""Run the long pressure-driven LASD case and plot its neutral log law."""
...
CONFIG = ROOT / "runners" / "pressure_driven_warmup" / "config.toml"
DEFAULT_OUTPUT = ROOT / "outputs" / "pressure_driven_lasd_64x64x64_gpu"
...
def _arguments(
    argv: list[str] | None,
    *,
    program: str = "python -m benchmark.PressureDrivenLASD",
    ...
    parser.add_argument(
        "--dt",
        type=float,
        help="override time.dt_seconds from config.toml",
    )
    parser.add_argument(
        "--hours",
        type=float,
